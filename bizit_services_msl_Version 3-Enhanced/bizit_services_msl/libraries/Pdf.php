...
class Pdf extends TCPDF {
	...
	function __construct($orientation = 'P', $unit = 'mm', $format = 'A4', $unicode = true, $encoding = 'UTF-8', $diskcache = false, $pdfa = false, $pdf_type = '') {
		...
		hooks()->do_action('pdf_construct', array('pdf_instance' => $this, 'type' => $this->pdf_type));
	}
	...
	public function Footer() {
		...
		hooks()->do_action('pdf_footer', array('pdf_instance' => $this, 'type' => $this->pdf_type));

		$this->SetFont('helvetica', 'I', 8);
		if (get_option('show_page_number_on_pdf') == 1 && !$this->last_page_flag) {
			$this->SetTextColor(142, 142, 142);
			$this->Cell(0, 15, $this->getAliasNumPage() . '/' . $this->getAliasNbPages(), 0, false, 'C', 0, '', 0, false, 'T', 'M');
		}
		if ($this->last_page_flag && 1==0) {
			$footer = '<img src="' . base_url() . '/assets/images/pdf_templates/background/footer.png" />';
			$this->writeHTMLCell(210, '', 0, 297 - 52, $footer, 0, 1, false, true, 'L', false);
		}
	}
