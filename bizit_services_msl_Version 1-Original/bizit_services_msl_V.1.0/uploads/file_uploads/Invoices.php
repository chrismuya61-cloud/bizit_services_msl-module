...
class Invoices extends AdminController
{
    ...
    {
        if ($this->input->post()) {
            $invoice_data = hooks()->apply_filters('bizit_invoices_data', $this->input->post());
            if ($id == '') {
                if (!has_permission('invoices', '', 'create')) {
                    access_denied('invoices');
                }
    ...
    public function record_payment()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($id) {
                // Check and update rental agreement status if the invoice is fully paid
                $this->payments_model->update_rental_agreement_status($this->input->post('invoiceid'));
    ...
    public function delete_payment($id, $invoiceid)
    {
        ...
        $this->load->model('payments_model');
        $this->load->model('services_model'); // Load a model for service requests if not already loaded
        ...
        if ($response == true) {
            // Update the status in tblservice_request
            $this->db->set('status', 3)
                    ->where('invoice_rel_id', $invoiceid)
                    ->update('tblservice_request');
    ...
    public function pdf($id)
    {
        ...
        $invoice_number = format_invoice_number($invoice->id);
         ...
         // Fetch accessories
        $accessories = $this->invoices_model->get_service_and_accessory_details($id);
        $invoice->accessories = $accessories; // Attach to invoice object
