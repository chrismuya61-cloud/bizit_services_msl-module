    ...
    <div class="row">
        <div class="col-md-12">
            <div class="table-responsive">
                <?php
                // Display the main items table with a unique ID
                $items = get_items_table_data($invoice, 'invoice', 'html', true);
                $invoice_table = $items->table();

                // Add an ID to the table for specific targeting
                $invoice_table = str_replace('<table', '<table id="invoice-items-table"', $invoice_table);
                echo $invoice_table;

                // Fetch accessory details
                $accessories = $this->invoices_model->get_service_and_accessory_details($invoice->id);

                if (!empty($accessories)) {
                    // Prepare accessory data as JSON
                    $accessory_data = [];
                    foreach ($accessories as $accessory) {
                        $qty = 1; // Assuming quantity is 1
                        $rate = $accessory->accessory_price;
                        $amount = $qty * $rate;

                        $accessory_data[] = [
                            'name' => htmlspecialchars($accessory->accessory_name),
                            'description' => htmlspecialchars($accessory->long_description),
                            'qty' => $qty,
                    ...
                    $json_accessories = json_encode($accessory_data);
                ?>
            
                <script>
                    (function() {
                        // Target the specific invoice table by its ID
                        const tableBody = document.querySelector("#invoice-items-table tbody");
                        const accessories = <?php echo $json_accessories; ?>;

                        // Count existing rows to continue serial numbering
                        let counter = tableBody.querySelectorAll('tr').length + 1;

                        accessories.forEach(function(accessory) {
                            let newRow = document.createElement("tr");
                            const accessoryLabel = accessory.qty > 1 ? 'accessories' : 'accessory'; // Handle pluralization

                            newRow.innerHTML = `
                                <td style="text-align: left;">#&nbsp;&nbsp;${counter++}</td>
                                <td>${accessory.name}</td>
                                <td style="width: 150px; text-align: center;">${accessory.qty}</td>
                                <td style="width: 150px; text-align: right;">${accessory.rate}</td>
                                <td style="width: 150px; text-align: right;">0%</td> <!-- Tax column -->
                                <td style="width: 150px; text-align: right;">${accessory.amount}</td>
                            `;
                ...
                <?php
                    ...
                    echo '<p>No equipment spares attached for this service.</p>';
                }
            ?>
    </div>
