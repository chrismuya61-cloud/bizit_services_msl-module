...
class Invoices_model extends App_Model
{
    ...
    public function get_service_and_accessory_details($invoice_id)
    {
        $this->db->select('
            sr.service_request_id,
            sr.service_request_code,
            a.id AS accessory_id,
            a.service_request_id AS accessory_service_request_id,
            a.price AS accessory_price,
            i.description AS accessory_name,
            i.long_description
        ');
        $this->db->from('tblservice_request_accessories AS a');
        $this->db->join('tblitems AS i', 'a.accessory_id = i.id', 'left');
        $this->db->join('tblservice_request AS sr', 'a.service_request_id = sr.service_request_id', 'left');
        $this->db->where('sr.invoice_rel_id', $invoice_id); // Added condition to match invoice_rel_id with invoice_id
        
        $query = $this->db->get();
        
        return $query->result(); // Return result of the query
    }
